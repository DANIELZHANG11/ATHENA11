# 07 - 部署与 SRE 手册 (DevOps & SRE Manual)
...
- **PostgreSQL** (`pg_data`): 数据库事务日志，IOPS 敏感（包含 pgvector 向量索引）
...
| 组件 | 规格 | 用途 |
|------|------|------|
| **CPU** | Intel Xeon E5-2680 v4 (14核28线程 @ 2.4-3.3GHz) | 高并发处理 |
| **内存** | 海力士 (Hynix) DDR4 ECC 2133MHz 32GB × 2 (共 64GB) | 应用 + 数据库缓存 |
| **主板** | Intel C612 芯片组 | 服务器级稳定性 |
| **GPU** | NVIDIA RTX 3060 (12GB GDDR6) | OCR + Embedding 模型推理 |
| **RAID 卡** | LSI MegaRAID 9271-8i (1GB 缓存) | 硬件 RAID 加速 |
| **系统盘** | RAID 1 SSD 480GB × 2 (可用 480GB) | 系统 + 高性能数据 |
| **静态文件盘** | RAID 5 HDD 3TB × 3 (可用 ~5.5TB) | MinIO 对象存储 |
| **只读缓存盘** | M.2 NVMe SSD 256GB | 应用层只读缓存 (bcache) |
| **网络** | 千兆以太网 | 内网穿透 (FRP) |

> **当前负载**：服务器同时运行另一个 Docker 化 Web 项目（日均 3 万 PV），当前占用约 15GB 内存、CPU < 10%、GPU 空闲。

#### Celery Worker 队列配置

> **设计原则**：基于服务器资源情况，合理分配 CPU/GPU 并发，付费任务优先处理。不设置硬性超时，保证任务最终完成。

| 队列名称 | 用途 | 基础并发 | 最大并发 | 资源限制 | 优先级 |
|---------|------|---------|---------|---------|--------|
| `conversion` | Calibre 格式转换 | 2 | 4 | 每任务 ≤ 2GB 内存 | 无 |
| `ocr_paid` | 付费 OCR | 2 | 3 | GPU 12GB 共享 | **高** |
| `ocr_free` | 免费 OCR | 1 | 2 | GPU 12GB 共享 | 低 |
| `indexing` | 向量索引 | 2 | 4 | CPU 密集型 | 无 |

**超时配置**：
| 任务类型 | 超时时间 | 超时处理 |
|---------|---------|---------|
| Calibre 转换 | 5 分钟 | 标记 `failed` + `processing_error='timeout'` |
| OCR 处理 | 30 分钟 | 标记 `failed` + `processing_error='timeout'` |
| 向量索引 | 10 分钟 | 标记 `failed` + 允许重试 |
...
| **当前负载** (与其他项目共存) | < 10% | 35-45 GB | 6-8 GB | 1.2-1.8 TB |
...
#### 持久化卷策略
**高性能存储 (SSD)**: `/home/vitiana/Athena/data_ssd/`
*   `pg_data`: PostgreSQL 数据（包含 pgvector 向量索引）
...
- **强制升级检查 (Force Upgrade)**:
    - 检查请求头 `X-Client-Version`。
    - 若低于最小版本 (如 `1.0.0`)，直接返回 `426 Upgrade Required`。
...
*   **存储位置**: 包含在 PostgreSQL 数据库中 (`pg_data`)
*   **备份方式**: 随 PostgreSQL 全量备份一起备份
*   **恢复**: 随数据库恢复，或可从原始数据重建索引
...
*   **排查**:
    ...
    # 2. 检查 pgvector 扩展是否启用
    docker compose exec postgres psql -U athena -d athena -c "SELECT * FROM pg_extension WHERE extname = 'vector';"
    
    # 3. 检查向量表状态
    docker compose exec postgres psql -U athena -d athena -c "SELECT COUNT(*) FROM vectors;"
    
    # 4. 检查 HNSW 索引状态
    docker compose exec postgres psql -U athena -d athena -c "SELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'vectors';"
    
    # 5. 查看 PostgreSQL 日志
    docker compose logs postgres --tail=100
    ```
*   **解决方案**:
    - 重建 HNSW 索引: `REINDEX INDEX CONCURRENTLY index_vectors_on_content_vector_hnsw;`
    - 增加 PostgreSQL 内存: `shared_buffers`, `work_mem`
    - 重新生成向量: `docker compose exec api python -m scripts.rebuild_vectors`
...
├── postgres/              # PostgreSQL (包含 pgvector 向量索引)
├── valkey/                # Redis
└── hf_cache/              # 模型缓存
...
4. 逐个启动 Athena 服务：
   ...
   docker compose up -d seaweed  # 启动对象存储层
