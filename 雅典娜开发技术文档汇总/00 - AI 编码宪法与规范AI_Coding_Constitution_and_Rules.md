# 00 - AI 编码宪法与规范 (AI Coding Constitution & Rules)
...
## 1. 核心指令 (Meta-Instructions)

**在执行任何任务之前，你必须首先确认以下原则：**
1.  **上下文完整性**：你正在构建一个**全新的 App-First SaaS 系统**，代号"雅典娜"。所有代码和架构必须严格遵循 00-07 号技术规范。
2.  **离线优先 (Local-First)**：前端开发的默认假设是“设备当前无网络”。UI 必须直接响应本地数据库的变化，而不是等待网络请求。
3.  **后端基石**：后端代码（API/DB）是数据安全和多端同步的最终真理。**API 设计和数据库表结构必须严格遵循本规范，禁止随意偏离。**
...
## 2. 离线优先铁律 (The App-First Prime Directives)
...
### 4. 同步防抖与本地缓存 (Debounce & Local Cache)
*   **网络防抖**：PowerSync 自动上传虽然实时，但对于高频操作（如阅读进度更新），前端必须实现 **Throttling (节流)**（如 30s 一次），防止电池与流量消耗。
*   **本地资产缓存**：对于 S3 Presigned URL（易过期），前端必须实现**本地文件缓存**（OPFS/Filesystem），优先读取 `file://` 或 `blob:`，严禁每次都从网络加载图片。
...
### 5. 移动端 Schema 迁移原则 (Mobile Schema Migration)
*   **只增不减 (Additive Only)**：鉴于移动端更新滞后，SQLite Schema 变更必须向后兼容。
*   **禁止**：删除字段、重命名字段、修改字段数据类型（如 TEXT -> INTEGER）。
*   **允许**：新增 Nullable 字段、新增表。

---

## 3. API 与 PowerSync 责任分工 (API vs PowerSync Responsibility Matrix)
...
## 4. 垂直切片工作流 (Vertical Slice Workflow)

1.  **对照规范 (Check Specifications)**：
    *   对照 `02_Functional_Specifications_PRD.md` 确认功能需求。
...
## 5. 数据安全与多租户 (Data Security & Multi-tenancy)
...
## 6. 技术强制约束 (Technical Enforcements)
...
### 6. 数据主权以人为本 (Data Sovereignty)
*   **严禁数据丢失**: 当 API 返回 403 (只读锁/配额超限) 时，前端**必须**捕获错误并保留本地数据，标记为"待同步"或"同步受阻"，**严禁**自动回滚或删除用户已创作的内容 (笔记/高亮)。
*   **逃生舱机制**: 系统必须提供不依赖网络的"导出全部数据"功能 (Markdown/JSON)，确保用户拥有数据主权。

---

## 7. 前端与设计系统规范 (Design System Rules)
...
## 8. AI 交互行为准则 (AI Behavior Guidelines)
...
## 9. 代码组织与模块化规范 (Code Organization & Modularity)

> **核心原则**：严禁单文件行数过长（>500行），严禁功能混杂。

### 1. 后端任务分离 (Backend Tasks)
*   **反面模式**：将所有 Celery 任务写在单个 `tasks.py` 文件中，导致膨胀为单体巨石。
*   **规则**：必须按业务领域拆分为 Python Package。
*   **结构**：
    ```
    api/app/tasks/
    ├── __init__.py        # 导出 celery_app
    ├── ocr.py             # OCR 相关任务
    ├── conversion.py      # 格式转换任务
    ├── vector.py          # 向量索引任务
    └── maintenance.py     # 清理与维护任务
    ```

### 2. 前端阅读器分离 (Frontend Reader Separation)
*   **规则**：不同格式的阅读器核心逻辑必须物理分离，严禁写在一个文件中。
*   **结构**：
    ```
    web/src/components/Reader/
    ├── PDFReader.tsx      # PDF 专用逻辑 (Canvas/PDF.js)
    ├── EpubReader.tsx     # EPUB 专用逻辑 (Iframe/React-Reader)
    ├── BaseReader.tsx     # 共享 UI (菜单/进度条)
    └── hooks/             # 阅读器专用 Hooks
    ```

### 3. 书籍卡片视图分离 (Book Card Views)
*   **规则**：不同展示形态（网格/列表/Hero）必须拆分为独立组件。
*   **结构**：
    ```
    web/src/components/BookCard/
    ├── BookCardGrid.tsx   # 竖向封面卡片 (Library 视图)
    ├── BookCardList.tsx   # 横向详细列表 (Admin/Search 视图)
    ├── BookCardHero.tsx   # 首页大图推荐 (Hero 视图)
    └── BookCover.tsx      # 共享封面组件 (处理图片加载/Fallback)
    ```

### 4. 书籍逻辑分离 (Book Logic Separation)
*   **规则**：书籍相关的数据 Hook 必须按功能域拆分，**严禁使用单体 useBook()**。
*   **拆分**：
    *   `useBookMeta(id)`: 仅标题、作者、封面
    *   `useBookContent(id)`: 仅文本内容与章节
    *   `useBookNotes(id)`: 仅笔记与高亮
    *   `useBookProgress(id)`: 仅阅读进度与 CFI
    *   `useBookAI(id)`: 仅 AI 对话上下文

### 5. 书籍处理流水线分离 (Book Processing Pipeline)
*   **规则**：鉴于上传逻辑复杂，必须将不同格式的处理逻辑物理分离，严禁全部堆在 `services/book.py`。
*   **结构**：
    ```
    api/app/services/book_processing/
    ├── __init__.py
    ├── manager.py         # 编排层 (Orchestrator): 决定使用哪个 Pipeline
    ├── pipeline_pdf.py    # PDF 专用流水线 (DRM Check / Metadata / Page Sampling)
    ├── pipeline_epub.py   # EPUB 专用流水线 (Fast Metadata)
    ├── pipeline_convert.py # 格式转换流水线 (MOBI/AZW3 -> EPUB)
    └── utils.py           # 共享工具 (DRM Check, Hash)
    ```
*   **原则**：
    *   **接口统一**：每个 Pipeline 必须实现统一接口 `process(file_path) -> ProcessingResult`。
    *   **异常隔离**：转换失败不应导致服务崩溃，必须捕获并返回 `processing_status='failed'`。

---

## 10. 附录 A：标准错误码表 (Standard Error Codes)
...
## 11. 附录 B：CI 检查清单 (Checklist for PR)
